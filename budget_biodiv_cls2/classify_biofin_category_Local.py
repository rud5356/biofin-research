...
INPUT_GLOB         = "20260708_부산본청_세부사업별 세출현황.csv"
...
def row_value(row: dict, logical_column: str) -> str:
    """Return a cell by logical name, allowing old and local CSV column aliases."""
...
def build_key(row: dict) -> str:
    return "\x1f".join(row_value(row, col) for col in KEY_COLUMNS)
...
def build_prompt(row: dict) -> str:
    cols = (
        "지역", "자치단체", "회계", "분야", "부문명",
        "프로그램명", "단위사업명", "세부사업명",
        "예산현액_계", "예산현액_국비", "예산현액_시도비",
        "예산현액_시군구비", "예산현액_기타", "지출액", "집행잔액",
    )
    values = {col: row_value(row, col) for col in cols}
    return PROMPT_TEMPLATE.format(**values)


def make_unique_headers(headers: list[str]) -> list[str]:
    """CSV DictWriter needs unique headers; duplicated names are suffixed."""
    seen: dict[str, int] = {}
    result: list[str] = []
    for index, raw_header in enumerate(headers, start=1):
        header = clean_cell(raw_header).lstrip("\ufeff") or f"column_{index}"
...
def looks_like_two_row_header(first: list[str], second: list[str]) -> bool:
    """Detect the local 부산 CSV shape: group header row + detail header row."""
    first_clean = [clean_cell(value).lstrip("\ufeff") for value in first]
    second_clean = [clean_cell(value).lstrip("\ufeff") for value in second]
    return (
        "세부사업명" in first_clean
        and "세부사업명" in second_clean
        and "예산현액" in first_clean
        and any(value in second_clean for value in ("계", "국비", "시도비", "시군구비", "기타"))
    )


def merge_two_row_headers(first: list[str], second: list[str]) -> list[str]:
    """Flatten two-level budget headers into stable single-row names.

    Example: 예산현액 + 국비 -> 예산현액_국비.
    The local CSV's last column is labelled 국비 but contains sector values, so
    after 예산현액_국비 already exists we normalize that bare 국비 to 부문명.
    """
    merged: list[str] = []
    max_len = max(len(first), len(second))
    for index in range(max_len):
        top = clean_cell(first[index]).lstrip("\ufeff") if index < len(first) else ""
        sub = clean_cell(second[index]).lstrip("\ufeff") if index < len(second) else ""
        if top == "예산현액" and sub:
            merged.append(f"예산현액_{sub}")
        elif top and sub and top != sub and sub not in {"계", "국비", "시도비", "시군구비", "기타"}:
            merged.append(sub)
        else:
            merged.append(sub or top or f"column_{index + 1}")

    normalized = make_unique_headers(merged)
    if "예산현액_국비" in normalized and "분야" in normalized:
        normalized = ["부문명" if header == "국비" else header for header in normalized]
    return make_unique_headers(normalized)
...
def read_csv(path: Path) -> tuple[list[str], list[dict]]:
    for enc in ENCODINGS:
        try:
            ...
            raw_rows = [row for row in raw_rows if any(clean_cell(cell) for cell in row)]
            if not raw_rows:
                return [], []

            if len(raw_rows) >= 2 and looks_like_two_row_header(raw_rows[0], raw_rows[1]):
                headers = merge_two_row_headers(raw_rows[0], raw_rows[1])
                data_rows = raw_rows[2:]
            else:
                headers = make_unique_headers(raw_rows[0])
...
def load_cache(path: Path) -> dict[str, dict]:
    ...
    # category=-1 means a previous timeout/parse failure, not a real label.
    # Ignore it so reruns can try the row again.
    return {
        r["hash"]: r
        for r in rows
        if r.get("hash") and str(r.get("category", "")).strip() != "-1"
    }
...
def row_needs_classification(row: dict, biodiv_col: str) -> bool:
    """Old labeled CSV uses biodiv_label=1; local CSV has no such column."""
    if not biodiv_col or biodiv_col not in row:
        return True
    return str(row.get(biodiv_col, "0")).strip() == "1"


def main() -> int:
    args = parse_args()

    cache_path = args.cache_csv or (args.output_dir / "category_cache_local.csv")
    ...
    # 기존 labeled CSV는 biodiv_label=1만 분류하고, 새 지방재정 CSV처럼
    # biodiv_label 컬럼이 없으면 모든 행을 BIOFIN category 대상으로 본다.
    ...
    for path in input_files:
        ...
        has_biodiv_col = args.biodiv_col in headers
        if not has_biodiv_col:
            print(f"  - {path.name}: {args.biodiv_col} 컬럼 없음 → 전체 행 분류")
        for row in rows:
            if not row_needs_classification(row, args.biodiv_col):
                continue
    ...
    print(f"\n분류 대상 행: {total_candidate_rows}개")
    print(f"분류 대상 고유 사업: {total_unique}개")
    ...
    with ThreadPoolExecutor(max_workers=args.workers) as executor:
        ...
        for future in as_completed(futures):
            ...
            if cat == -1:
                errors += 1
                reason = clean_cell(result.get("reason", ""))[:120]
                status = f"오류: {reason}"
            else:
                status = f"category={cat}"

            # Timeout/parse failures are often transient.  Do not cache category
            # -1 results, otherwise a temporary Ollama failure will be reused as
            # a permanent classification result on later runs.
            if cat != -1:
                with cache_lock:
                    cache[h] = {"hash": h, **result}

            done += 1
            row_repr = row_value(unique_rows[h], "세부사업명")[:40]
