...
def save_evaluation_outputs(
    ...
    output_dir: str | Path,
    num_labels: int = 10,
    label_codes: dict[int, str] | None = None,
) -> dict[str, float]:
    """최종 test 결과와 0~9 고정 축을 저장한다."""
    output_dir = Path(output_dir)
    output_dir.mkdir(parents=True, exist_ok=True)
    prediction_frame = pd.DataFrame(prediction_rows)
    if prediction_frame.empty:
        raise ValueError("저장할 test prediction이 없습니다")
    ...
    label_codes = label_codes or {label: str(label) for label in all_labels}
    prediction_frame["true_subcategory"] = prediction_frame["true_label"].map(label_codes)
    prediction_frame["pred_subcategory"] = prediction_frame["pred_label"].map(label_codes)

    report_text = classification_report(
        y_true,
        y_pred,
        labels=all_labels,
        target_names=[label_codes[label] for label in all_labels],
        digits=4,
        zero_division=0,
    )
    ...
    matrix_frame = pd.DataFrame(
        matrix,
        index=[f"true_{label_codes[label]}" for label in all_labels],
        columns=[f"pred_{label_codes[label]}" for label in all_labels],
    )
    matrix_frame.index.name = "true_label"
    matrix_frame.to_csv(output_dir / "confusion_matrix.csv", encoding="utf-8-sig")
    write_csv(prediction_frame, output_dir / "test_predictions.csv")
    write_csv(attention_rows, output_dir / "test_attention_outputs.csv")
    return {
