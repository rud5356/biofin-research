...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--label_column", default="하위 카테고리")
    parser.add_argument(
        "--result_column",
        default="예측 하위 카테고리",
        help="원본 CSV에 추가할 예측 결과 컬럼명",
    )
    parser.add_argument(
        "--classified_filename",
        default="classified.csv",
        help="원본 컬럼과 예측 결과를 합쳐 저장할 파일명",
    )
    parser.add_argument(
        "--filename_column",
        default="사업설명자료_파일명",
...
def load_label_mapping(model_dir: str | Path) -> dict[str, int]:
    path = Path(model_dir) / "label_map.json"
    if not path.is_file():
        raise FileNotFoundError(f"하위 카테고리 label map이 없습니다: {path}")
    with path.open("r", encoding="utf-8") as handle:
        payload = json.load(handle)
    mapping = {str(code): int(class_id) for code, class_id in payload["code_to_id"].items()}
    if not mapping:
        raise ValueError("label_map.json의 code_to_id가 비어 있습니다")
    return mapping
...
def predict_records(
    ...
    label_mapping = checkpoint.get("label_mapping") or load_label_mapping(model_dir)
    id_to_code = {int(class_id): str(code) for code, class_id in label_mapping.items()}
        ...
        for batch_number, batch in enumerate(loader, start=1):
            ...
            for item_index, meta in enumerate(batch["meta"]):
                ...
                row = {
                    ...
                    "pred_subcategory": id_to_code.get(pred_label, "알 수 없음"),
...
def save_classified_budget(
    budget: pd.DataFrame,
    prediction_rows: list[dict[str, Any]],
    output_path: str | Path,
    result_column: str,
) -> None:
    """원본 예산 컬럼을 보존하고 매칭된 행에 예측 번호 하나를 추가한다."""

    predictions_by_row = {
        int(row["source_row"]): str(row["pred_subcategory"])
        for row in prediction_rows
    }
    internal_columns = [column for column in budget.columns if column.startswith("_")]
    output = budget.drop(columns=internal_columns, errors="ignore").copy()
    source_rows = budget["_source_row"].astype(int)
    output[result_column] = source_rows.map(predictions_by_row)
...
def save_optional_evaluation(
    ...
    id_to_code = id_to_code or {label: str(label) for label in labels}
    report = classification_report(
        y_true,
        y_pred,
        labels=labels,
        target_names=[id_to_code.get(label, str(label)) for label in labels],
        digits=4,
        zero_division=0,
    )
    ...
    matrix_frame = pd.DataFrame(
        matrix,
        index=[f"true_{id_to_code.get(label, label)}" for label in labels],
        columns=[f"pred_{id_to_code.get(label, label)}" for label in labels],
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        label_mapping = load_label_mapping(args.model_dir)
        ...
        save_classified_budget(
            budget,
            predictions,
            output_dir / args.classified_filename,
            args.result_column,
        )
        id_to_code = {
            int(class_id): code
            for code, class_id in label_mapping.items()
        }
        summary = save_optional_evaluation(
            predictions,
            output_dir,
            num_labels,
