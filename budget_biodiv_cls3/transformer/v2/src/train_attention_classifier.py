...
DEFAULT_OUTPUT_DIR = Path(__file__).resolve().parents[1] / "outputs" / "subcategory_model"
...
def build_argument_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help="0이면 데이터에서 하위 카테고리 수를 자동 결정",
    )
...
def validate_arguments(args: argparse.Namespace) -> None:
    ...
    if args.num_labels < 0:
        raise ValueError("num_labels는 0 이상이어야 합니다")
...
def train(args: argparse.Namespace, records: list[dict[str, Any]], output_dir: Path) -> int:
    ...
    reported_metrics = save_evaluation_outputs(
        ...
        num_labels=args.num_labels,
        label_codes={
            int(class_id): code for code, class_id in args.label_mapping.items()
        },
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        labels, label_files, label_mapping = load_label_data(
            args.label_file,
            label_column=args.label_column,
            num_labels=args.num_labels,
        )
        args.label_mapping = label_mapping
        args.num_labels = len(label_mapping)
        write_json(
            {
                "label_column": args.label_column,
                "num_labels": args.num_labels,
        ...
        LOGGER.info(
            "하위 카테고리 %d종을 모델 class ID 0~%d로 인코딩했습니다.",
            args.num_labels,
            args.num_labels - 1,
        )
