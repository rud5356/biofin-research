...
def load_label_data(
    ...
    raw_label = labels[label_column]
    missing_label = raw_label.isna() | raw_label.astype(str).str.strip().eq("")
    numeric_label = pd.to_numeric(raw_label, errors="coerce")
    numeric_label.loc[missing_label] = 0
    ...
    labels.loc[numeric_label.isna(), "_label_status"] = "INVALID_LABEL"
    ...
    missing_count = int(missing_label.sum())
    if invalid_count:
        LOGGER.warning(
    ...
    if missing_count:
        LOGGER.info(
            "%s이 비어 있는 행 %d개를 1차 카테고리 0으로 분류합니다.",
            label_column,
            missing_count,
        )
...
def match_documents_to_labels(
    ...
    """명시적 문서 경로를 우선하고, 없으면 연도/소관/사업명으로 매칭한다."""
    groups: dict[tuple[int, str], list[int]] = defaultdict(list)
    for index, row in labels.iterrows():
        if pd.notna(row["_year"]) and row["_ministry_norm"]:
            groups[(int(row["_year"]), row["_ministry_norm"])].append(index)

    filename_groups: dict[str, set[int]] = defaultdict(set)
    for column in ("사업설명자료_상대경로", "사업설명자료_파일명", "matched_filename"):
        if column not in labels.columns:
            continue
        for index, value in labels[column].items():
            if value is None or pd.isna(value) or not str(value).strip():
                continue
            filename_groups[Path(str(value).strip()).name.casefold()].add(int(index))
    ...
    for path in documents:
        ...
        if identity is None:
            ...
            continue
        explicit = sorted(filename_groups.get(path.name.casefold(), set()))
        if explicit:
            label_rows_with_document.update(explicit)
            if len(explicit) > 1:
                explicit_labels = {
                    int(labels.at[idx, "_label"])
                    for idx in explicit
                ...
                if len(explicit_labels) == 1:
                    ...
                    failures.append(
                        _failure_row(
                            "MULTIPLE_LABEL_ROWS",
                            f"{len(explicit)}개 문서 경로 일치(source rows: {rows})",
                            identity,
                        )
                    )
                    continue
            ...
            if row["_label_status"] != "OK":
                ...
                failures.append(
                    _failure_row(reason, "명시적 문서 매칭 행의 label 오류", identity, row)
                )
                continue
            successes.append(
                {
