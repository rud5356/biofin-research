...
PROMPT_VERSION = "kr-biofin-subcategory-2026-08-19-flat-v5"
VALID_LABELS = (
    "0",
    "1.01", "1.04", "1.05",
...
사업정보와 사업설명자료만으로 비해당 "0" 또는 전체 하위 카테고리 중
핵심 목적과 가장 구체적으로 일치하는 코드 하나를 직접 선택한다. 정답 라벨이나
1차 카테고리는 입력으로 제공되지 않는다. 상위 번호 1~9만 출력해서는 안 된다.
...
- 수질·토양 오염은 6.01, 하수관로·하수처리장·폐기물은 6.03,
  해양오염은 6.04로 분류한다.
...
- 먼저 사업이 BIOFIN에 해당하는지 판단한 뒤, 해당하면 전체 하위 코드 가운데
  핵심 목적과 가장 구체적으로 일치하는 코드 하나를 선택한다.
...
6. 오염 관리
- 사람의 건강·위생만을 위한 정화는 제외한다. 6.06은 생물다양성·생태계 보호 목적이 명시돼야 하며, 나머지는 오염의 직접적인 생태계 영향으로 간접 편익을 인정할 수 있다.
...
- 7.04 이동성·멸종위기·보호 야생종의 현지 내 보호와 보전조치. 가축·재배종은 9.01로 분류한다.
...
사업이 BIOFIN 분류 기준에 해당하지 않으면 "0"을 선택한다. 해당하면
상위 범주 1~9와 세부 활동을 함께 판단하여 전체 허용 하위 코드 중 하나를
직접 선택한다. 정답 라벨을 추정하거나 입력에 없는 활동을 만들어내지 않는다.
