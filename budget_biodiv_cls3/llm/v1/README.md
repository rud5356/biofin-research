...
사업설명자료는 텍스트로 변환한 뒤 `사업목적` 또는 `목적` 제목부터
다음 `사업내용`, `추진근거` 등 항목 제목 전까지의 내용만 사용합니다.
문서 전체 본문은 LLM에 전달하지 않습니다. 목적 항목은 앞에서 최대 16,000자를 사용합니다.
`document_status=PURPOSE_EXTRACTED`이면 목적 추출 성공,
`PURPOSE_NOT_FOUND`이면 목적 제목이나 종료 경계를 찾지 못한 것입니다.
이 경우 문서 본문 없이 예산 메타데이터만으로 분류합니다.
`사업목적·내용`, `사업목적 및 내용` 등 결합 항목도 사용하며, 다음 `사업개요` 또는 `사업근거 및 추진경위` 등 제목 앞에서 종료합니다.
목적 전용 프롬프트 버전을 사용하므로 이전 전체 본문 기반 캐시는 재사용하지 않습니다.
...
요청에는 `think: false`를 지정하여 최종 JSON 답변을 받도록 합니다.
`thinking`만 반환되더라도 이를 최종 예측으로 사용하지 않습니다.
서버·모델 변경 후에는 `--limit-keys 1 --workers 1 --retries 0`으로 먼저 검증합니다.
