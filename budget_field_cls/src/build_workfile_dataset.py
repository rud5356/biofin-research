"""
매칭된 예산 작업표(workfile)에서 파일 분류 데이터셋을 구축하는 스크립트.

동작 순서:
  1. 작업표(CSV/XLSX) 읽기 — 분야명, 세부사업명, matched_filename 컬럼 필수
...
출력 파일:
  workfile_dataset.csv       : 파일별 학습 데이터 (라벨 포함)
  workfile_labels.csv        : 라벨 ID → 라벨 이름 매핑
  workfile_dataset_summary.json : 처리 통계
  workfile_dataset_unresolved.csv: 파일 경로 해결 실패 행

실행 예:
    python src/build_workfile_dataset.py
    python src/build_workfile_dataset.py --budget-root /path/to/data
"""
...
# ─── 기본 경로 설정 ───────────────────────────────────────────────────────────
# 환경변수 BUDGET_ROOT 가 있으면 사용, 없으면 기본 경로
...
# 포함할 자동 매칭 상태 (exact=완전일치, manual=수동, normalized=정규화 매칭)
DEFAULT_INCLUDE_STATUSES = ("exact", "manual", "normalized")
DEFAULT_OUTPUT_CSV       = DATA_DIR / "workfile_dataset.csv"
DEFAULT_LABELS_CSV       = DATA_DIR / "workfile_labels.csv"
DEFAULT_SUMMARY_JSON     = DATA_DIR / "workfile_dataset_summary.json"
DEFAULT_UNRESOLVED_CSV   = DATA_DIR / "workfile_dataset_unresolved.csv"
# 유효하지 않은 파일명 플레이스홀더 (이 값이면 해결 불가로 처리)
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="매칭된 예산 작업표에서 파일 분류 데이터셋을 구축합니다."
    )
    parser.add_argument("--workfile",      type=Path, default=DEFAULT_WORKFILE,     help="작업표 파일 경로 (.xlsx, .csv, .tsv)")
    parser.add_argument("--sheet-name",               default=DEFAULT_SHEET_NAME,   help="Excel 시트 이름")
    parser.add_argument("--budget-root",   type=Path, default=DEFAULT_BUDGET_ROOT,  help="분야 폴더가 있는 예산 데이터 루트 폴더")
    parser.add_argument("--output-csv",    type=Path, default=DEFAULT_OUTPUT_CSV,   help="데이터셋 CSV 저장 경로")
    parser.add_argument("--labels-csv",    type=Path, default=DEFAULT_LABELS_CSV,   help="라벨 매핑 CSV 저장 경로")
    parser.add_argument("--summary-json",  type=Path, default=DEFAULT_SUMMARY_JSON, help="요약 JSON 저장 경로")
    parser.add_argument("--unresolved-csv",type=Path, default=DEFAULT_UNRESOLVED_CSV, help="미해결 행 CSV 저장 경로")
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="같은 파일 경로를 가리키는 중복 행을 유지합니다 (기본: 첫 번째만 유지)",
    )
...
def read_workfile(workfile_path: Path, sheet_name: str) -> pd.DataFrame:
    """
    작업표 파일을 읽어 DataFrame으로 반환합니다.

    지원 형식: .csv/.tsv (cp949 + 탭 구분자), .xlsx (Excel)
    필수 컬럼: 분야명, 세부사업명, matched_filename
    auto_match_status 컬럼이 없으면 'manual' 로 채웁니다.
    """
    if not workfile_path.exists():
        raise FileNotFoundError(f"작업표 파일을 찾을 수 없습니다: {workfile_path}")

    suffix = workfile_path.suffix.lower()
    if suffix in {".csv", ".tsv"}:
        # 한글 Windows 기본 인코딩(cp949), 탭 구분자
        dataframe = pd.read_csv(workfile_path, sep="\t", encoding="cp949")
    else:
        dataframe = pd.read_excel(workfile_path, sheet_name=sheet_name)

    required_columns = {"분야명", "세부사업명", "matched_filename"}
    missing_columns  = required_columns - set(dataframe.columns)
    if missing_columns:
        raise ValueError(f"작업표에 필수 컬럼이 없습니다: {', '.join(sorted(missing_columns))}")

    # auto_match_status 컬럼이 없는 경우 수동 매칭으로 간주
    if "auto_match_status" not in dataframe.columns:
        dataframe["auto_match_status"] = "manual"
...
def scan_files(
    ...
    동일한 파일명이 여러 폴더에 존재할 수 있으므로 목록으로 관리합니다.
    (예: 여러 분야 폴더에 같은 이름의 파일이 있는 경우)
    """
    # 확장자 정규화 (앞에 점이 없으면 붙여줌)
    ...
    if not budget_root.exists():
        raise FileNotFoundError(f"예산 데이터 루트 폴더를 찾을 수 없습니다: {budget_root}")
...
def resolve_file_path(
    ...
    해결 순서:
    1. 파일명이 1개뿐이면 → unique_filename
    2. 파일명이 여러 개이면 분야명과 상위 폴더명이 일치하는 것 선택
       - 1개 일치 → filename_and_field_folder
       - 2개 이상 일치 → multiple_candidates_same_field (실패)
    3. 분야 폴더로도 구분 불가 → multiple_candidates_across_folders (실패)

    Returns:
        (해결된 Path 또는 None, 해결 상태 문자열)
    """
...
def safe_relative_path(path: Path, root: Path) -> str:
    """path 가 root 내에 있으면 상대 경로, 없으면 절대 경로를 문자열로 반환합니다."""
    try:
...
def build_dataset_rows(
    ...
    """
    작업표 각 행에서 파일 경로를 해결하고 데이터셋 행을 구성합니다.

    Returns:
        (dataset_rows: 성공 행 목록, unresolved_rows: 실패 행 목록)
    """
...
def drop_duplicate_paths(rows: list[dict[str, object]]) -> list[dict[str, object]]:
    """
    같은 file_path 를 가리키는 중복 행을 제거합니다 (처음 등장한 행만 유지).

    동일 파일이 여러 사업에 매칭된 경우 데이터 누출 방지를 위해 제거합니다.
    """
...
def assign_label_ids(rows: list[dict[str, object]]) -> list[dict[str, object]]:
    """
    라벨 문자열에 정수 ID를 부여합니다 (알파벳 순 정렬).

    각 행에 label_id 컬럼을 추가하고,
    라벨 ID → 라벨 이름 매핑 목록을 반환합니다.
    """
...
def run(args: argparse.Namespace) -> int:
    ...
    unresolved_csv = args.unresolved_csv.resolve()
